...
pub export fn sa_node_plugin_readline_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"clearLine\":true,\"clearScreenDown\":true,\"cursorTo\":true,\"moveCursor\":true,\"emitKeypressEvents\":true,\"promises\":true,\"createInterface\":true,\"Interface\":false,\"completer\":false,\"historyEditing\":false},\"capabilities\":[\"terminal cursor and clear helpers over file descriptors\",\"emitKeypressEvents compatibility stub\",\"readline.promises explicit Interface handle with question, close, snapshot, and free helpers\",\"createInterface compatibility entry point returning the same native Interface handle model\"],\"limitations\":[\"no JavaScript readline.Interface class instances\",\"createInterface returns an explicit native handle rather than a callback-driven EventEmitter object\",\"no completer, history editing, raw-mode line editor, or terminal redraw behavior\",\"promises support is limited to explicit native handle operations rather than JavaScript Promise/Interface objects\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_readline_config_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"cursorModel\":\"fd-based clear and cursor movement helpers\",\"promisesInterfaceModel\":\"explicit native Interface handle backed by buffered input text\",\"createInterfaceModel\":\"top-level createInterface reuses the same explicit native Interface handle as readline.promises\",\"keypressModel\":\"emitKeypressEvents compatibility stub\",\"objectModel\":\"not-modeled for JavaScript Interface instances\"}");
}

pub export fn sa_node_plugin_readline_feature_support_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"clearLine\":{\"supported\":true,\"mode\":\"fd-based ANSI clear-line helper\"},\"clearScreenDown\":{\"supported\":true,\"mode\":\"fd-based ANSI clear-screen-down helper\"},\"cursorTo\":{\"supported\":true,\"mode\":\"fd-based ANSI cursor positioning helper\"},\"moveCursor\":{\"supported\":true,\"mode\":\"fd-based ANSI relative cursor movement helper\"},\"emitKeypressEvents\":{\"supported\":true,\"mode\":\"compatibility stub with no event object production\"},\"promises\":{\"supported\":true,\"mode\":\"explicit native Interface handle with create/question/close/snapshot/free helpers\"},\"createInterface\":{\"supported\":true,\"mode\":\"top-level compatibility entry point returning the same explicit native Interface handle used by readline.promises\",\"limitations\":[\"does not construct a JavaScript readline.Interface EventEmitter instance\",\"follow-up operations use the explicit native handle helpers rather than callback events\"]},\"Interface\":{\"supported\":false,\"reason\":\"JavaScript readline Interface class instances are not modeled\"},\"completer\":{\"supported\":false,\"reason\":\"custom completion callbacks are not modeled\"},\"historyEditing\":{\"supported\":false,\"reason\":\"interactive line editing and history semantics are not modeled\"}}");
}

pub export fn sa_node_plugin_readline_create_interface(input_ptr: ?[*]const u8, input_len: u64, out_interface: ?*?*anyopaque) u32 {
    return ext.sa_node_plugin_readline_promises_create_interface(input_ptr, input_len, out_interface);
}
...
pub export fn sa_node_plugin_console_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"log\":true,\"info\":true,\"debug\":true,\"warn\":true,\"error\":true,\"dir\":true,\"dirxml\":true,\"table\":true,\"trace\":true,\"assert\":true,\"count\":true,\"countReset\":true,\"group\":true,\"groupCollapsed\":true,\"groupEnd\":true,\"time\":true,\"timeEnd\":true,\"timeLog\":true,\"timeStamp\":true,\"clear\":true,\"Console\":false},\"capabilities\":[\"global console-style stdout/stderr native text helpers\",\"native count and countReset label registry\",\"native timer-label registry for time, timeEnd, and timeLog helper flows\",\"terminal clear plus compatibility helpers for dir, dirxml, table, trace, group, and timeStamp\"],\"limitations\":[\"no JavaScript Console class instances or custom stream-backed console construction\",\"no inspect option, colorMode, or group indentation object-model parity\",\"time, timeEnd, and clear are explicit native helpers rather than stream-backed Console instance methods\",\"formatting follows this plugin's native text helpers rather than exact Node util.format and inspector behavior\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_fs_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"access\":true,\"exists\":true,\"stat\":true,\"lstat\":true,\"readdir\":true,\"readFile\":true,\"writeFile\":true,\"mkdir\":true,\"rmdir\":true,\"rm\":true,\"unlink\":true,\"rename\":true,\"copyFile\":true,\"cp\":true,\"realpath\":true,\"readlink\":true,\"symlink\":true,\"truncate\":true,\"utimes\":true,\"opendir\":true,\"open\":true,\"statfs\":true,\"glob\":true,\"promises\":true,\"constants\":true,\"watch\":false,\"watchFile\":false,\"unwatchFile\":false,\"ReadStream\":false,\"WriteStream\":false,\"Dir\":false,\"Dirent\":false,\"Stats\":false,\"openAsBlob\":false},\"capabilities\":[\"sync-style native file, directory, metadata, link, timestamp, truncate, glob, cp, and statfs helpers\",\"fs.promises compatibility entry points returning already-resolved native results for the same path and fd operations\",\"explicit opendir handle operations for directory iteration\",\"fs constants metadata through the native constants aggregate\"],\"limitations\":[\"no JavaScript ReadStream, WriteStream, Dir, Dirent, or Stats class instances\",\"no callback scheduling or event-emitter watch semantics\",\"promises returns already-resolved native results rather than JavaScript Promise objects\",\"cp supports explicit native file and recursive directory copies, but not JavaScript option objects, filters, AbortSignal, or timestamp/mode edge-case parity\",\"symlink does not model the JavaScript type argument or Windows link-type behaviors\",\"watch, watchFile, unwatchFile, and openAsBlob are not modeled at the top-level facade\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_fs_config_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"syncModel\":\"explicit native file and directory helpers returning immediate status plus buffers or scalars\",\"promisesModel\":\"fs.promises compatibility entry points returning already-resolved native results for the same path and fd helpers\",\"cpModel\":\"explicit file and recursive directory copy helper with force and error-on-exist flags\",\"dirModel\":\"explicit opendir handle with next and free operations\",\"fdModel\":\"numeric file descriptors with explicit read/write/fstat/fsync/fdatasync/ftruncate/futimes helpers\",\"pathMutationModel\":\"symlink, truncate, and utimes map directly to native filesystem mutations over host paths\",\"constantsModel\":\"native constants aggregate including fs access and copyfile flags\",\"objectModel\":\"not-modeled for JavaScript stream, Dir, Dirent, or Stats instances\"}");
}

pub export fn sa_node_plugin_fs_feature_support_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"access\":{\"supported\":true,\"mode\":\"native existence and mode check helper returning boolean result\"},\"exists\":{\"supported\":true,\"mode\":\"native boolean existence helper\"},\"stat\":{\"supported\":true,\"mode\":\"native stat JSON helper\"},\"lstat\":{\"supported\":true,\"mode\":\"native lstat JSON helper\"},\"readdir\":{\"supported\":true,\"mode\":\"native directory listing JSON helper\"},\"readFile\":{\"supported\":true,\"mode\":\"native full-file read helper returning buffer\"},\"writeFile\":{\"supported\":true,\"mode\":\"native full-file write helper\"},\"mkdir\":{\"supported\":true,\"mode\":\"native mkdir helper with recursive flag\"},\"rmdir\":{\"supported\":true,\"mode\":\"native directory removal helper\"},\"rm\":{\"supported\":true,\"mode\":\"native remove helper with recursive flag\"},\"unlink\":{\"supported\":true,\"mode\":\"native unlink helper\"},\"rename\":{\"supported\":true,\"mode\":\"native rename helper\"},\"copyFile\":{\"supported\":true,\"mode\":\"native copy-file helper\"},\"cp\":{\"supported\":true,\"mode\":\"native file and recursive directory copy helper with force and error-on-exist flags\",\"limitations\":[\"does not expose JavaScript options objects, filter callbacks, AbortSignal, timestamp preservation, or Promise object identity\"]},\"realpath\":{\"supported\":true,\"mode\":\"native realpath helper returning resolved path text\"},\"readlink\":{\"supported\":true,\"mode\":\"native readlink helper returning target path text\"},\"symlink\":{\"supported\":true,\"mode\":\"native symlink helper over host paths\",\"limitations\":[\"does not expose the JavaScript type argument or Windows link-type resolution behavior\"]},\"truncate\":{\"supported\":true,\"mode\":\"native truncate helper over host paths\",\"limitations\":[\"accepts explicit byte length only and does not model callback or Promise object identity semantics\"]},\"utimes\":{\"supported\":true,\"mode\":\"native utimes helper over host paths using millisecond timestamps\",\"limitations\":[\"does not model JavaScript Date object coercion semantics\"]},\"opendir\":{\"supported\":true,\"mode\":\"explicit native directory handle with next and free operations\"},\"open\":{\"supported\":true,\"mode\":\"native open helper returning numeric file descriptor\"},\"statfs\":{\"supported\":true,\"mode\":\"native statfs JSON helper\"},\"glob\":{\"supported\":true,\"mode\":\"native glob helper returning matched path JSON\"},\"promises\":{\"supported\":true,\"mode\":\"fs.promises compatibility entry points over the same native operations including symlink, truncate, and utimes\",\"limitations\":[\"returns already-resolved native results rather than JavaScript Promise object identity or microtask timing\"]},\"constants\":{\"supported\":true,\"mode\":\"native constants aggregate with fs access and copyfile flags\"},\"watch\":{\"supported\":false,\"reason\":\"FSWatcher event-emitter semantics are not modeled\"},\"watchFile\":{\"supported\":false,\"reason\":\"stat polling watcher semantics are not modeled\"},\"unwatchFile\":{\"supported\":false,\"reason\":\"stat polling watcher teardown semantics are not modeled\"},\"ReadStream\":{\"supported\":false,\"reason\":\"JavaScript ReadStream class instances are not modeled\"},\"WriteStream\":{\"supported\":false,\"reason\":\"JavaScript WriteStream class instances are not modeled\"},\"Dir\":{\"supported\":false,\"reason\":\"JavaScript Dir class instances are not modeled; use explicit native opendir handles instead\"},\"Dirent\":{\"supported\":false,\"reason\":\"JavaScript Dirent class instances are not modeled; directory entries are returned as JSON or scalar metadata\"},\"Stats\":{\"supported\":false,\"reason\":\"JavaScript Stats class instances are not modeled; stat data is returned as JSON\"},\"openAsBlob\":{\"supported\":false,\"reason\":\"Blob construction and stream-backed file blob semantics are not modeled\"}}");
}
...
pub export fn sa_node_plugin_crypto_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"randomBytes\":true,\"randomFill\":true,\"randomInt\":true,\"randomUUID\":true,\"createHash\":true,\"createHmac\":true,\"pbkdf2\":true,\"hkdf\":true,\"scrypt\":true,\"createCipheriv\":true,\"createDecipheriv\":true,\"sign\":true,\"verify\":true,\"generateKey\":true,\"getHashes\":true,\"timingSafeEqual\":true,\"secureHeapUsed\":true,\"webcrypto\":true,\"subtle\":true,\"generateKeyPair\":false,\"createPrivateKey\":false,\"createPublicKey\":false,\"createSecretKey\":false,\"KeyObject\":false,\"HashClass\":false,\"HmacClass\":false,\"Certificate\":false,\"X509Certificate\":false},\"capabilities\":[\"native random byte, UUID, integer, and fill helpers\",\"explicit native hash, HMAC, cipher, and decipher state handles\",\"PBKDF2, HKDF, scrypt, sign, verify, and symmetric key-generation helpers\",\"native secure heap usage snapshot matching Node's disabled-secure-heap shape\",\"native hash catalog metadata and Web Crypto sync subset\"],\"limitations\":[\"no JavaScript Hash, Hmac, Cipheriv, Decipheriv, KeyObject, Certificate, or X509Certificate class instances\",\"no callback scheduling or Promise object identity for async-style Node crypto APIs\",\"generateKeyPair and asymmetric key object construction are not modeled at the top-level facade\",\"secureHeapUsed reports the disabled secure-heap status snapshot because this plugin does not configure or manage OpenSSL secure heap memory\",\"webcrypto and subtle are exposed as native sync subsets rather than browser-compatible object graphs\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_util_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"callbackify\":true,\"promisify\":true,\"debuglog\":true,\"deprecate\":true,\"format\":true,\"formatWithOptions\":true,\"styleText\":true,\"inherits\":true,\"inspect\":true,\"isDeepStrictEqual\":true,\"stripVTControlCharacters\":true,\"parseArgs\":true,\"diff\":true,\"MIMEType\":true,\"MIMEParams\":false,\"getCallSites\":false,\"getSystemErrorMap\":true,\"getSystemErrorName\":true,\"getSystemErrorMessage\":true,\"TextDecoder\":true,\"TextEncoder\":true,\"toUSVString\":true,\"types\":false,\"parseEnv\":true,\"setTraceSigInt\":false,\"transferableAbortSignal\":false,\"transferableAbortController\":false,\"aborted\":false},\"capabilities\":[\"native util.format and util.formatWithOptions formatting helpers over JSON argument arrays\",\"native util.inspect style JSON formatting helpers\",\"native callbackify and promisify metadata wrappers plus deprecation and debuglog helpers\",\"native parseArgs, parseEnv, diff, MIME type lookup, styleText passthrough, and VT control stripping helpers\",\"native UTF-8 TextEncoder/TextDecoder-compatible encode, decode, and USV-style text normalization helpers\",\"native system error name, message, and error-map helpers reusing the existing errors surface\",\"deep strict equality and inheritance compatibility helpers without JavaScript prototype mutation semantics\"],\"limitations\":[\"no JavaScript class constructors or lazy property getters for MIMEParams or util.types\",\"TextEncoder, TextDecoder, and toUSVString are exposed as explicit native UTF-8 helpers rather than JavaScript class instances, typed arrays, string coercion from arbitrary values, streaming decoders, or BOM/fatal option objects\",\"MIMEType is exposed as native path-to-media-type lookup metadata rather than a WHATWG MIMEType class instance\",\"formatWithOptions validates the inspect options JSON but currently formats through the native util.format subset\",\"getCallSites, setTraceSigInt, and abort-transfer helpers are not modeled\",\"callbackify and promisify expose native wrapper metadata rather than JavaScript function identity, callback timing, or Promise object semantics\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_util_config_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"formatModel\":\"native util.format and util.formatWithOptions helpers over JSON text and printf-style placeholders\",\"inspectOptionsModel\":\"formatWithOptions validates options JSON but currently formats through the native util.format subset\",\"wrapModel\":\"callbackify and promisify return native wrapper metadata rather than JavaScript callable wrappers\",\"argModel\":\"parseArgs and parseEnv return already-resolved native JSON values rather than JavaScript objects\",\"textCodecModel\":\"explicit native UTF-8 TextEncoder/TextDecoder-compatible helpers returning owned byte/text buffers\",\"textNormalizationModel\":\"toUSVString validates native UTF-8 text input and returns a well-formed UTF-8 buffer suitable for USV-style string handling without JavaScript coercion semantics\",\"mimeModel\":\"MIMEType compatibility is modeled as filename/path media-type lookup JSON rather than WHATWG MIME classes\",\"errorModel\":\"system error helpers reuse the native errors surface for errno-name, errno-message, and code-map JSON\",\"deprecationModel\":\"native deprecation registry with explicit codes and messages\",\"objectModel\":\"not-modeled for JavaScript util.types or MIMEType/MIMEParams class instances; TextEncoder/TextDecoder are helper functions rather than constructible classes\"}");
}

pub export fn sa_node_plugin_util_feature_support_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"callbackify\":{\"supported\":true,\"mode\":\"native metadata wrapper describing callbackified function intent\",\"limitations\":[\"does not return a JavaScript callable wrapper or nextTick rejection timing\"]},\"promisify\":{\"supported\":true,\"mode\":\"native metadata wrapper describing promisified function intent\",\"limitations\":[\"does not return a JavaScript Promise-returning wrapper function\"]},\"debuglog\":{\"supported\":true,\"mode\":\"NODE_DEBUG-aware native section metadata helper\"},\"deprecate\":{\"supported\":true,\"mode\":\"native deprecation registry keyed by code\",\"limitations\":[\"does not wrap JavaScript functions or emit process warning events\"]},\"format\":{\"supported\":true,\"mode\":\"native printf-style formatter over JSON argument arrays\"},\"formatWithOptions\":{\"supported\":true,\"mode\":\"native format helper accepting and validating inspect options JSON\",\"limitations\":[\"inspect options do not currently alter output beyond the native util.format subset\"]},\"styleText\":{\"supported\":true,\"mode\":\"native passthrough helper preserving input text\",\"limitations\":[\"ANSI style expansion and stream validation are not modeled\"]},\"inherits\":{\"supported\":true,\"mode\":\"native compatibility stub for inheritance intent\",\"limitations\":[\"does not mutate JavaScript prototypes\"]},\"inspect\":{\"supported\":true,\"mode\":\"native pretty-printed JSON formatter\",\"limitations\":[\"does not implement Node inspect custom hooks, colors, getters, or prototype traversal semantics\"]},\"isDeepStrictEqual\":{\"supported\":true,\"mode\":\"native JSON deep strict equality helper\"},\"stripVTControlCharacters\":{\"supported\":true,\"mode\":\"native VT control stripping helper\"},\"parseArgs\":{\"supported\":true,\"mode\":\"native JSON parseArgs subset for long --key=value options and positionals\",\"limitations\":[\"does not implement the full Node option schema, tokens, short-option, strict, or default-value behavior\"]},\"parseEnv\":{\"supported\":true,\"mode\":\"native dotenv-style parser returning flat JSON key-value pairs\"},\"diff\":{\"supported\":true,\"mode\":\"native replace-or-empty diff summary helper\"},\"MIMEType\":{\"supported\":true,\"mode\":\"native filename/path to media-type lookup helper\",\"limitations\":[\"does not construct a WHATWG MIMEType class instance\"]},\"MIMEParams\":{\"supported\":false,\"reason\":\"WHATWG MIME parameter collection semantics are not modeled\"},\"getCallSites\":{\"supported\":false,\"reason\":\"JavaScript stack frame and source-map call-site inspection requires runtime integration\"},\"getSystemErrorMap\":{\"supported\":true,\"mode\":\"native JSON map of Node-style and system errno codes reusing the errors surface\"},\"getSystemErrorName\":{\"supported\":true,\"mode\":\"native errno-to-name helper reusing the errors surface\"},\"getSystemErrorMessage\":{\"supported\":true,\"mode\":\"native errno-to-message helper reusing the errors surface\"},\"TextDecoder\":{\"supported\":true,\"mode\":\"native UTF-8 decode helper over byte slices\",\"limitations\":[\"not a JavaScript TextDecoder class instance\",\"streaming state, BOM handling, fatal option objects, and non-UTF encodings are not modeled\"]},\"TextEncoder\":{\"supported\":true,\"mode\":\"native UTF-8 encode helper over text slices\",\"limitations\":[\"not a JavaScript TextEncoder class instance or Uint8Array object\",\"encodeInto destination-array object semantics are not modeled\"]},\"toUSVString\":{\"supported\":true,\"mode\":\"native UTF-8 well-formed text normalization helper over text slices\",\"limitations\":[\"accepts native UTF-8 text input rather than arbitrary JavaScript values\",\"does not model JavaScript string coercion or object identity semantics\"]},\"types\":{\"supported\":false,\"reason\":\"util.types object and predicate catalog are not modeled\"},\"setTraceSigInt\":{\"supported\":false,\"reason\":\"SIGINT trace integration is not modeled\"},\"transferableAbortSignal\":{\"supported\":false,\"reason\":\"AbortSignal transfer helpers require JavaScript object semantics\"},\"transferableAbortController\":{\"supported\":false,\"reason\":\"AbortController transfer helpers require JavaScript object semantics\"},\"aborted\":{\"supported\":false,\"reason\":\"abort state helper requires JavaScript AbortSignal semantics\"}}");
}

fn utilIsUtf8EncodingLabel(label: []const u8) bool {
    return label.len == 0 or
        std.ascii.eqlIgnoreCase(label, "utf-8") or
        std.ascii.eqlIgnoreCase(label, "utf8") or
        std.ascii.eqlIgnoreCase(label, "unicode-1-1-utf-8");
}

pub export fn sa_node_plugin_util_text_encoder_encode(text_ptr: ?[*]const u8, text_len: u64, out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    const text = (text_ptr orelse return fail())[0..text_len];
    if (!std.unicode.utf8ValidateSlice(text)) return fail();
    return writeOwnedBytes(out_ptr, out_len, text);
}

pub export fn sa_node_plugin_util_text_decoder_decode(data_ptr: ?[*]const u8, data_len: u64, encoding_ptr: ?[*]const u8, encoding_len: u64, out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    const data = (data_ptr orelse return fail())[0..data_len];
    const encoding = if (encoding_len == 0) "" else (encoding_ptr orelse return fail())[0..encoding_len];
    if (!utilIsUtf8EncodingLabel(encoding)) return fail();
    if (!std.unicode.utf8ValidateSlice(data)) return fail();
    return writeOwnedBytes(out_ptr, out_len, data);
}

pub export fn sa_node_plugin_util_to_usv_string(input_ptr: ?[*]const u8, input_len: u64, out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    const input = (input_ptr orelse return fail())[0..input_len];
    var view = std.unicode.Utf8View.init(input) catch return fail();
    var it = view.iterator();
    var out = std.ArrayList(u8).init(std.heap.page_allocator);
    defer out.deinit();
    while (it.nextCodepoint()) |cp| {
        const normalized_cp: u21 = if (cp >= 0xD800 and cp <= 0xDFFF) 0xFFFD else cp;
        var buf: [4]u8 = undefined;
        const n = std.unicode.utf8Encode(normalized_cp, &buf) catch return fail();
        out.appendSlice(buf[0..n]) catch return fail();
    }
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_buffer_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"transcode\":true,\"isUtf8\":true,\"isAscii\":true,\"atob\":true,\"btoa\":true,\"resolveObjectURL\":true,\"Buffer\":false,\"Buffer_byteLength\":true,\"Buffer_concat\":true,\"Blob\":false,\"File\":false,\"constants\":true,\"kMaxLength\":true,\"kStringMaxLength\":true,\"INSPECT_MAX_BYTES\":false},\"capabilities\":[\"native base64 helpers, ASCII and UTF-8 classification, and transcoding across common encodings\",\"native blob URL normalization through resolveObjectURL\",\"native byteLength, concat, constants, kMaxLength, and kStringMaxLength helpers exposed through the buffer ABI\",\"top-level export-name and support metadata for the common public buffer surface\"],\"limitations\":[\"no JavaScript Buffer class instances, prototype methods, pooling, or Uint8Array subclass semantics\",\"no JavaScript Blob or File class instances or object URL store semantics\",\"mutable INSPECT_MAX_BYTES getter/setter state is not modeled; only the default value is included in constants JSON\",\"Buffer static methods are available as native ABI helpers rather than through a live Buffer constructor export\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_buffer_constants_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"MAX_LENGTH\":9007199254740991,\"MAX_STRING_LENGTH\":536870888,\"kMaxLength\":9007199254740991,\"kStringMaxLength\":536870888,\"INSPECT_MAX_BYTES_DEFAULT\":50}");
}

pub export fn sa_node_plugin_buffer_config_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"textModel\":\"native base64 and encoding transcode helpers over raw byte slices\",\"classificationModel\":\"native UTF-8 and ASCII predicates over raw byte slices\",\"objectUrlModel\":\"resolveObjectURL strips the blob: wrapper to a native underlying URL string\",\"bufferStaticModel\":\"byteLength, concat, constants, kMaxLength, and kStringMaxLength are exposed as direct native helpers rather than through a JavaScript Buffer constructor\",\"constantsModel\":\"Node v26-compatible read-only constants JSON snapshot with INSPECT_MAX_BYTES default metadata\",\"objectModel\":\"not-modeled for JavaScript Buffer, Blob, or File class instances\"}");
}

pub export fn sa_node_plugin_buffer_feature_support_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"transcode\":{\"supported\":true,\"mode\":\"native transcoding helper across utf8, utf16le, latin1, ascii, base64, base64url, and hex\"},\"isUtf8\":{\"supported\":true,\"mode\":\"native UTF-8 validation helper\"},\"isAscii\":{\"supported\":true,\"mode\":\"native ASCII validation helper\"},\"atob\":{\"supported\":true,\"mode\":\"native base64 decode helper returning text bytes\"},\"btoa\":{\"supported\":true,\"mode\":\"native base64 encode helper from text bytes\"},\"resolveObjectURL\":{\"supported\":true,\"mode\":\"native blob URL normalization helper\",\"limitations\":[\"does not consult a JavaScript blob URL registry or return Blob objects\"]},\"Buffer_byteLength\":{\"supported\":true,\"mode\":\"native byte-length helper over raw slices\"},\"Buffer_concat\":{\"supported\":true,\"mode\":\"native concatenation helper over explicit slice arrays\"},\"constants\":{\"supported\":true,\"mode\":\"native read-only constants JSON snapshot containing MAX_LENGTH and MAX_STRING_LENGTH\"},\"kMaxLength\":{\"supported\":true,\"mode\":\"native read-only constant metadata matching the constants.MAX_LENGTH snapshot\"},\"kStringMaxLength\":{\"supported\":true,\"mode\":\"native read-only constant metadata matching the constants.MAX_STRING_LENGTH snapshot\"},\"Buffer\":{\"supported\":false,\"reason\":\"JavaScript Buffer constructor, Uint8Array subclass identity, and prototype methods are not modeled\"},\"Blob\":{\"supported\":false,\"reason\":\"JavaScript Blob class instances are not modeled in the buffer facade\"},\"File\":{\"supported\":false,\"reason\":\"JavaScript File class instances are not modeled in the buffer facade\"},\"INSPECT_MAX_BYTES\":{\"supported\":false,\"reason\":\"mutable util.inspect Buffer truncation state is not modeled; the constants snapshot exposes only the default value\"}}");
}
...
pub export fn sa_node_plugin_url_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"parse\":true,\"format\":true,\"resolve\":true,\"URL\":true,\"URLSearchParams\":false,\"domainToASCII\":true,\"domainToUnicode\":true,\"pathToFileURL\":true,\"fileURLToPath\":true,\"fileURLToPathBuffer\":true,\"URLPattern\":false,\"urlToHttpOptions\":true,\"canParse\":true},\"capabilities\":[\"legacy-style native url.parse, url.format, and url.resolve helpers over JSON and string inputs\",\"explicit native URL handle creation plus href, protocol, host, pathname, and urlToHttpOptions getters\",\"native domainToASCII/domainToUnicode helpers via existing punycode conversion support\",\"native pathToFileURL, fileURLToPath, fileURLToPathBuffer, and canParse helpers\",\"top-level export-name and support metadata for the public url module surface\"],\"limitations\":[\"no WHATWG URLSearchParams, URLPattern, or live JavaScript URL class object graph semantics\",\"urlToHttpOptions operates on the explicit native URL handle rather than a JavaScript WHATWG URL instance\",\"parse/format/resolve and file URL helpers follow this plugin's native subset and do not claim full WHATWG object-model parity\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_process_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"pid\":true,\"ppid\":true,\"cwd\":true,\"uptime\":true,\"hrtime\":true,\"cpuUsage\":true,\"memoryUsage\":true,\"argv\":true,\"argv0\":true,\"execArgv\":true,\"execPath\":true,\"versions\":true,\"env\":true,\"geteuid\":true,\"getegid\":true,\"getuid\":true,\"getgid\":true,\"getgroups\":true,\"kill\":true,\"resourceUsage\":true,\"availableMemory\":true,\"constrainedMemory\":true,\"features\":true,\"exit\":true,\"arch\":true,\"platform\":true,\"release\":true,\"version\":true,\"allowedNodeEnvironmentFlags\":true,\"versionsObjectIdentity\":false,\"stdin\":false,\"stdout\":false,\"stderr\":false,\"nextTick\":false,\"emitWarning\":true,\"dlopen\":false,\"umask\":true,\"chdir\":true},\"capabilities\":[\"native process identity, arch, platform, cwd, chdir, uptime, hrtime, argv, argv0, execPath, execArgv, version, release, versions, env, real and effective uid/gid, and supplemental group helpers\",\"native process cpuUsage, memoryUsage, resourceUsage, availableMemory, constrainedMemory, features, and umask helpers\",\"native allowedNodeEnvironmentFlags snapshot and membership helpers over the plugin's known NODE_OPTIONS-compatible flag set\",\"native warning JSON helper over message, type, code, and detail text using existing deprecation and no-warning flag introspection\",\"real POSIX signal delivery through kill helpers and explicit process exit\",\"top-level export-name and support metadata for the public process surface\"],\"limitations\":[\"no JavaScript EventEmitter process object semantics, warning events, or nextTick queue integration\",\"no live stdin/stdout/stderr stream objects or process.channel/message IPC behavior\",\"execArgv is a snapshot of recognized native host Node-style options and does not execute V8 or loader flags\",\"allowedNodeEnvironmentFlags is exposed as native JSON plus membership helpers rather than a JavaScript Set with full Node/V8 flag coverage\",\"process.dlopen is not exposed as a dedicated native helper in this facade\",\"versions, release, and env are exposed through explicit JSON and getter/setter helpers rather than live JavaScript object identity\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
fn processNormalizeAllowedNodeEnvironmentFlag(allocator: std.mem.Allocator, input: []const u8) ![]u8 {
    const trimmed = std.mem.trim(u8, input, " \t\r\n");
    if (trimmed.len == 0) return allocator.dupe(u8, "");
    const normalized_input = if (std.mem.indexOfScalar(u8, trimmed, '=')) |eq_idx| trimmed[0..eq_idx] else trimmed;
    if (std.mem.startsWith(u8, normalized_input, "--") or std.mem.startsWith(u8, normalized_input, "-")) {
        return allocator.dupe(u8, normalized_input);
    }
    if (normalized_input.len == 1) return std.fmt.allocPrint(allocator, "-{s}", .{normalized_input});
    return std.fmt.allocPrint(allocator, "--{s}", .{normalized_input});
}

fn processAllowedNodeEnvironmentFlagsHas(flag: []const u8) bool {
    const allocator = std.heap.page_allocator;
    const normalized = processNormalizeAllowedNodeEnvironmentFlag(allocator, flag) catch return false;
    defer allocator.free(normalized);
    if (normalized.len == 0) return false;
    for (process_allowed_node_environment_flags) |entry| {
        if (std.mem.eql(u8, normalized, entry)) return true;
    }
    return false;
}

pub export fn sa_node_plugin_process_allowed_node_environment_flags_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    var out = std.ArrayList(u8).init(std.heap.page_allocator);
    defer out.deinit();
    appendStringArray(&out, &process_allowed_node_environment_flags) catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}

pub export fn sa_node_plugin_process_allowed_node_environment_flags_has(flag_ptr: ?[*]const u8, flag_len: u64, out_bool: ?*u64) u32 {
...
pub export fn sa_node_plugin_process_exec_path(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    const path = std.fs.selfExePath(&buffer) catch return fail();
    return writeOwnedString(out_ptr, out_len, path);
}

pub export fn sa_node_plugin_process_argv0(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    var config = commandLineOptionsReadConfig(std.heap.page_allocator) catch return fail();
    defer config.deinit();
    if (config.argv.items.len != 0 and config.argv.items[0].len != 0) return writeOwnedString(out_ptr, out_len, config.argv.items[0]);

    if (std.fs.openFileAbsolute("/proc/self/cmdline", .{})) |file| {
        defer file.close();
        var buffer: [4096]u8 = undefined;
        const n = file.readAll(&buffer) catch 0;
        if (n != 0) {
            var it = std.mem.splitScalar(u8, buffer[0..n], '\x00');
            while (it.next()) |arg| {
                if (arg.len != 0) return writeOwnedString(out_ptr, out_len, arg);
            }
        }
    } else |_| {}

    return sa_node_plugin_process_exec_path(out_ptr, out_len);
}

pub export fn sa_node_plugin_process_exec_argv_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    var config = commandLineOptionsReadConfig(std.heap.page_allocator) catch return fail();
    defer config.deinit();
    var out = std.ArrayList(u8).init(std.heap.page_allocator);
    defer out.deinit();
    appendOwnedStringArray(&out, config.exec_argv_tokens.items) catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_os_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"arch\":true,\"availableParallelism\":true,\"cpus\":true,\"endianness\":true,\"freemem\":true,\"getPriority\":true,\"homedir\":true,\"hostname\":true,\"loadavg\":true,\"machine\":true,\"networkInterfaces\":true,\"platform\":true,\"release\":true,\"setPriority\":true,\"tmpdir\":true,\"totalmem\":true,\"type\":true,\"uptime\":true,\"userInfo\":true,\"version\":true,\"constants\":true,\"devNull\":true,\"EOL\":true},\"capabilities\":[\"native CPU, memory, load average, uptime, and network-interface snapshots\",\"native platform, arch, release, type, version, machine, hostname, tmpdir, homedir, endianness, EOL, and devNull helpers\",\"native os.constants aggregate plus process-priority get/set helpers\",\"top-level export-name and support metadata for the public os module surface\"],\"limitations\":[\"platform and arch follow the plugin's native host reporting rather than Node's exact cross-build target matrix semantics\",\"cpus and networkInterfaces return snapshot JSON rather than live JavaScript object graphs or typed class instances\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_path_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"join\":true,\"resolve\":true,\"normalize\":true,\"basename\":true,\"dirname\":true,\"extname\":true,\"isAbsolute\":true,\"relative\":true,\"format\":true,\"parse\":true,\"sep\":true,\"delimiter\":true,\"matchesGlob\":true,\"toNamespacedPath\":true,\"posix\":false,\"win32\":false},\"capabilities\":[\"native path join, resolve, normalize, basename, dirname, extname, and isAbsolute helpers\",\"native relative, format, parse, sep, delimiter, matchesGlob, and POSIX toNamespacedPath helpers\",\"top-level export-name and support metadata for the public path module surface\"],\"limitations\":[\"no separate path.posix or path.win32 namespace objects with platform-specific method tables\",\"toNamespacedPath is implemented as the POSIX no-op helper and does not claim Windows namespace conversion semantics\",\"normalization and resolution follow the host-native std.fs.path behavior rather than Node's full cross-platform edge-case matrix\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_string_decoder_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"StringDecoder\":true,\"write\":true,\"end\":true,\"text\":false,\"lastChar\":false,\"lastNeed\":false,\"lastTotal\":false},\"capabilities\":[\"native explicit StringDecoder handle allocation and free\",\"native write and end helpers that preserve or flush incomplete UTF-8 multibyte sequences across chunks\",\"top-level export-name and support metadata for the public string_decoder module surface\"],\"limitations\":[\"only the explicit native handle plus write/end/free subset is exposed\",\"text, lastChar, lastNeed, and lastTotal JavaScript instance semantics are not modeled\",\"encoding selection and broader non-UTF-8 codec families are not exposed as configurable JavaScript constructor behavior\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_string_decoder_config_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"handleModel\":\"explicit native StringDecoder handle allocated with create() and released with free()\",\"writeModel\":\"native write helper accumulates incomplete UTF-8 multibyte sequences between chunks\",\"endModel\":\"native end helper optionally writes a final chunk and flushes an incomplete buffered sequence as replacement bytes\",\"encodingModel\":\"current native implementation exposes a fixed UTF-8-oriented decoder subset rather than full JavaScript encoding selection\",\"objectModel\":\"not-modeled for JavaScript constructor instances, prototype getters, or text methods\"}");
}

pub export fn sa_node_plugin_string_decoder_feature_support_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return writeOwnedString(out_ptr, out_len, "{\"StringDecoder\":{\"supported\":true,\"mode\":\"explicit native handle with create, write, end, and free operations\"},\"write\":{\"supported\":true,\"mode\":\"native chunk decoder with incomplete UTF-8 sequence buffering\"},\"end\":{\"supported\":true,\"mode\":\"native final chunk decode plus pending UTF-8 flush helper\",\"limitations\":[\"flushes incomplete pending bytes as UTF-8 replacement bytes rather than exposing JavaScript internal fields\"]},\"text\":{\"supported\":false,\"reason\":\"legacy text(buf, offset) instance helper is not modeled\"},\"lastChar\":{\"supported\":false,\"reason\":\"JavaScript getter exposing the buffered incomplete character bytes is not modeled\"},\"lastNeed\":{\"supported\":false,\"reason\":\"JavaScript getter exposing missing byte count is not modeled\"},\"lastTotal\":{\"supported\":false,\"reason\":\"JavaScript getter exposing buffered plus missing byte count is not modeled\"}}");
}
...
pub export fn sa_node_plugin_zlib_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"gzipSync\":true,\"gunzipSync\":true,\"deflateSync\":true,\"inflateSync\":true,\"deflateRawSync\":true,\"inflateRawSync\":true,\"unzipSync\":true,\"brotliCompressSync\":true,\"brotliDecompressSync\":true,\"zstdCompressSync\":true,\"zstdDecompressSync\":true,\"crc32\":true,\"createGzip\":false,\"createGunzip\":false,\"createDeflate\":false,\"createInflate\":false,\"createDeflateRaw\":false,\"createInflateRaw\":false,\"createUnzip\":false,\"createBrotliCompress\":false,\"createBrotliDecompress\":false,\"createZstdCompress\":false,\"createZstdDecompress\":false,\"gzip\":false,\"gunzip\":false,\"deflate\":false,\"inflate\":false,\"deflateRaw\":false,\"inflateRaw\":false,\"unzip\":false,\"brotliCompress\":false,\"brotliDecompress\":false,\"zstdCompress\":false,\"zstdDecompress\":false,\"constants\":true,\"codes\":true},\"capabilities\":[\"native synchronous gzip, gunzip, deflate, inflate, raw-deflate, raw-inflate, and unzip helpers\",\"native synchronous Brotli and Zstd compress and decompress helpers plus CRC32\",\"static zlib constants and code-name lookup JSON snapshots\",\"top-level export-name and support metadata for the public zlib module surface\"],\"limitations\":[\"no JavaScript Transform stream classes or create* constructor helpers\",\"callback-style asynchronous convenience methods are not modeled; this facade exposes synchronous native helpers only\",\"constants and codes are exposed as JSON snapshots rather than frozen JavaScript objects\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
const CommandLineOptionsConfig = struct {
    ...
    exec_argv_tokens: std.ArrayList([]u8),
    ...
    fn init(allocator: std.mem.Allocator) CommandLineOptionsConfig {
        return .{
            ...
            .exec_argv_tokens = std.ArrayList([]u8).init(allocator),
    ...
    fn deinit(self: *CommandLineOptionsConfig) void {
        ...
        CommandLineOptionsConfig.freeItems(self.allocator, &self.exec_argv_tokens);
...
fn commandLineOptionsRecognizedToken(token: []const u8) bool {
    return std.mem.startsWith(u8, token, "--env-file") or
        std.mem.startsWith(u8, token, "--env-file-if-exists") or
        std.mem.startsWith(u8, token, "--require") or
        std.mem.eql(u8, token, "-r") or
        std.mem.startsWith(u8, token, "-r=") or
        std.mem.startsWith(u8, token, "--conditions") or
        std.mem.eql(u8, token, "-C") or
        std.mem.startsWith(u8, token, "-C=") or
        std.mem.eql(u8, token, "-e") or
        std.mem.eql(u8, token, "--eval") or
        std.mem.startsWith(u8, token, "--eval=") or
        std.mem.eql(u8, token, "-p") or
        std.mem.eql(u8, token, "--print") or
        std.mem.startsWith(u8, token, "--print=") or
        std.mem.eql(u8, token, "-c") or
        std.mem.eql(u8, token, "--check") or
        std.mem.eql(u8, token, "-i") or
        std.mem.eql(u8, token, "--interactive") or
        commandLineOptionsInspectPrefix(token) or
        std.mem.startsWith(u8, token, "--input-type") or
        std.mem.startsWith(u8, token, "--experimental-") or
        std.mem.startsWith(u8, token, "--trace-") or
        std.mem.eql(u8, token, "--enable-source-maps");
}

fn commandLineOptionsExecArgConsumesNext(token: []const u8) bool {
    return std.mem.eql(u8, token, "-e") or
        std.mem.eql(u8, token, "--eval") or
        std.mem.eql(u8, token, "-p") or
        std.mem.eql(u8, token, "--print");
}
...
fn commandLineOptionsParseArgv(config: *CommandLineOptionsConfig) !void {
    ...
    var i: usize = 1;
    var before_entrypoint = true;
    while (i < argv.len) : (i += 1) {
        const token = argv[i];
        const next = if (i + 1 < argv.len) argv[i + 1] else null;
        const record_exec_arg = before_entrypoint and commandLineOptionsRecognizedToken(token);
        const consumed_next = try commandLineOptionsParseToken(config, argv[i], if (i + 1 < argv.len) argv[i + 1] else null, false);
        if (record_exec_arg) {
            try commandLineOptionsPush(&config.exec_argv_tokens, allocator, token);
            if ((consumed_next or commandLineOptionsExecArgConsumesNext(token)) and next != null) try commandLineOptionsPush(&config.exec_argv_tokens, allocator, next.?);
        }
        if (before_entrypoint and (std.mem.eql(u8, token, "--") or !std.mem.startsWith(u8, token, "-"))) before_entrypoint = false;
...
pub export fn sa_node_plugin_util_parse_env(content_ptr: ?[*]const u8, content_len: u64, out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return sa_node_plugin_environment_variables_parse_env_json(content_ptr, content_len, out_ptr, out_len);
}

pub export fn sa_node_plugin_util_get_system_error_name(errnum: i64, out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return sa_node_plugin_errors_get_system_error_name(errnum, out_ptr, out_len);
}

pub export fn sa_node_plugin_util_get_system_error_message(errnum: i64, out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return sa_node_plugin_errors_get_system_error_message(errnum, out_ptr, out_len);
}

pub export fn sa_node_plugin_util_get_system_error_map(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    return sa_node_plugin_errors_codes_json(out_ptr, out_len);
}
...
pub export fn sa_node_plugin_net_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice("},\"featureSupport\":{\"connect\":true,\"createConnection\":true,\"createConnectionOptions\":true,\"listenOptions\":true,\"createServer\":true,\"isIP\":true,\"isIPv4\":true,\"isIPv6\":true,\"BlockList\":true,\"BlockList.familyArguments\":true,\"BlockList.isBlockList\":true,\"SocketAddress\":true,\"SocketAddress.isSocketAddress\":true,\"getDefaultAutoSelectFamily\":true,\"setDefaultAutoSelectFamily\":true,\"getDefaultAutoSelectFamilyAttemptTimeout\":true,\"setDefaultAutoSelectFamilyAttemptTimeout\":true,\"Server\":false,\"Socket\":false,\"Stream\":false,\"_createServerHandle\":false,\"_normalizeArgs\":false},\"capabilities\":[\"TCP and Unix socket connect/listen/accept/read/write/end helpers\",\"socket and server address, timeout, buffer, ref, readyState, and byte-counter metadata\",\"SocketAddress and BlockList native handles with static handle-type checks\",\"BlockList string helpers with Node-style family arguments and IPv4-mapped IPv6 normalization\",\"TCP connect blocklist filtering and createConnection/createServer convenience helpers\",\"createConnection JSON options for host, hostname, port, family, localAddress, localPort, noDelay, keepAlive, keepAliveInitialDelay, and timeoutMs\",\"listen JSON options for TCP host/port and Unix path using existing native listener helpers\",\"default auto-select-family setting metadata and setters\"],\"limitations\":[\"no JavaScript Server, Socket, or Stream class instances\",\"no EventEmitter callback object model for sockets or servers\",\"_createServerHandle and _normalizeArgs internal JavaScript helpers are not modeled\",\"createConnection and createServer return explicit native handles rather than JavaScript objects\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
...
pub export fn sa_node_plugin_net_config_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    var out = std.ArrayList(u8).init(allocator);
    defer out.deinit();
    out.appendSlice("{\"socketModel\":\"explicit native TCP or Unix socket handle\",\"serverModel\":\"explicit native listener handle\",\"blockListModel\":\"explicit native BlockList handle with Node-style family arguments\",\"socketAddressModel\":\"explicit native SocketAddress handle\",\"defaultAutoSelectFamily\":") catch return fail();
    out.appendSlice(if (auto_select_family != 0) "true" else "false") catch return fail();
