...
pub export fn sa_node_plugin_fs_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"access\":true,\"exists\":true,\"stat\":true,\"lstat\":true,\"readdir\":true,\"readFile\":true,\"writeFile\":true,\"mkdir\":true,\"rmdir\":true,\"rm\":true,\"unlink\":true,\"rename\":true,\"copyFile\":true,\"cp\":true,\"realpath\":true,\"readlink\":true,\"opendir\":true,\"open\":true,\"statfs\":true,\"glob\":true,\"promises\":true,\"constants\":true,\"watch\":false,\"watchFile\":false,\"unwatchFile\":false,\"ReadStream\":false,\"WriteStream\":false,\"Dir\":false,\"Dirent\":false,\"Stats\":false,\"openAsBlob\":false},\"capabilities\":[\"sync-style native file, directory, metadata, fd, link, glob, cp, and statfs helpers\",\"fs.promises compatibility entry points returning already-resolved native results\",\"explicit opendir handle operations for directory iteration\",\"fs constants metadata through the native constants aggregate\"],\"limitations\":[\"no JavaScript ReadStream, WriteStream, Dir, Dirent, or Stats class instances\",\"no callback scheduling or event-emitter watch semantics\",\"promises returns already-resolved native results rather than JavaScript Promise objects\",\"cp supports explicit native file and recursive directory copies, but not JavaScript option objects, filters, AbortSignal, or timestamp/mode edge-case parity\",\"watch, watchFile, unwatchFile, and openAsBlob are not modeled at the top-level facade\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
