...
pub export fn sa_node_plugin_process_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice(",\"featureSupport\":{\"pid\":true,\"ppid\":true,\"cwd\":true,\"uptime\":true,\"hrtime\":true,\"cpuUsage\":true,\"memoryUsage\":true,\"argv\":true,\"argv0\":true,\"execArgv\":true,\"execPath\":true,\"versions\":true,\"env\":true,\"geteuid\":true,\"getegid\":true,\"getuid\":true,\"getgid\":true,\"groups\":true,\"kill\":true,\"resourceUsage\":true,\"availableMemory\":true,\"constrainedMemory\":true,\"features\":true,\"exit\":true,\"arch\":true,\"platform\":true,\"release\":true,\"version\":true,\"allowedNodeEnvironmentFlags\":true,\"versionsObjectIdentity\":false,\"stdin\":false,\"stdout\":false,\"stderr\":false,\"nextTick\":false,\"emitWarning\":true,\"dlopen\":false,\"umask\":true,\"chdir\":true},\"capabilities\":[\"native process identity, arch, platform, cwd, chdir, uptime, hrtime, argv, argv0, execPath, execArgv, version, release, versions, env, real and effective uid/gid, and groups helpers\",\"native process cpuUsage, memoryUsage, resourceUsage, availableMemory, constrainedMemory, features, and umask helpers\",\"native allowedNodeEnvironmentFlags snapshot and membership helpers over the plugin's known NODE_OPTIONS-compatible flag set\",\"native warning JSON helper over message, type, code, and detail text using existing deprecation and no-warning flag introspection\",\"real POSIX signal delivery through kill helpers and explicit process exit\",\"top-level export-name and support metadata for the public process surface\"],\"limitations\":[\"no JavaScript EventEmitter process object semantics, warning events, or nextTick queue integration\",\"no live stdin/stdout/stderr stream objects or process.channel/message IPC behavior\",\"execArgv is a snapshot of recognized native host Node-style options and does not execute V8 or loader flags\",\"allowedNodeEnvironmentFlags is exposed as native JSON plus membership helpers rather than a JavaScript Set with full Node/V8 flag coverage\",\"process.dlopen is not exposed as a dedicated native helper in this facade\",\"versions, release, and env are exposed through explicit JSON and getter/setter helpers rather than live JavaScript object identity\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
