...
pub export fn sa_node_plugin_net_status_json(out_ptr: ?*?[*]const u8, out_len: ?*u64) u32 {
    ...
    out.appendSlice("},\"featureSupport\":{\"connect\":true,\"createConnection\":true,\"createConnectionOptions\":true,\"listenOptions\":true,\"createServer\":true,\"isIP\":true,\"isIPv4\":true,\"isIPv6\":true,\"BlockList\":true,\"BlockList.isBlockList\":true,\"SocketAddress\":true,\"SocketAddress.isSocketAddress\":true,\"getDefaultAutoSelectFamily\":true,\"setDefaultAutoSelectFamily\":true,\"getDefaultAutoSelectFamilyAttemptTimeout\":true,\"setDefaultAutoSelectFamilyAttemptTimeout\":true,\"Server\":false,\"Socket\":false,\"Stream\":false,\"_createServerHandle\":false,\"_normalizeArgs\":false},\"capabilities\":[\"TCP and Unix socket connect/listen/accept/read/write/end helpers\",\"socket and server address, timeout, buffer, ref, readyState, and byte-counter metadata\",\"SocketAddress and BlockList native handles with static handle-type checks\",\"TCP connect blocklist filtering and createConnection/createServer convenience helpers\",\"createConnection JSON options for host, hostname, port, family, localAddress, localPort, noDelay, keepAlive, keepAliveInitialDelay, and timeoutMs\",\"listen JSON options for TCP host/port and Unix path using existing native listener helpers\",\"default auto-select-family setting metadata and setters\"],\"limitations\":[\"no JavaScript Server, Socket, or Stream class instances\",\"no EventEmitter callback object model for sockets or servers\",\"_createServerHandle and _normalizeArgs internal JavaScript helpers are not modeled\",\"createConnection and createServer return explicit native handles rather than JavaScript objects\"]}") catch return fail();
    return writeOwnedBytes(out_ptr, out_len, out.items);
}
